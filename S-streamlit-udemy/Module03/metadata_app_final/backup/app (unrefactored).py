	...
	if choice == "Home":
		...
		if image_file is not None:
			...
			with st.beta_expander("File Stats"):
				...
				"FileSize":image_file.size,"FileType":image_file.type}
			...
			with c2:
				with st.beta_expander("Default(JPEG)"):
					...
					"filename":img.filename,"size":img.size,"height":img.height,"width":img.width,
					"info":img.info}
		...
		if audio_file is not None:
			...
			with col2:
				with st.beta_expander("File Stats"):
					...
					"FileSize":audio_file.size,"FileType":audio_file.type}
					st.write(file_details)
		...
		if text_file is not None:
			...
			with dcol1:
				with st.beta_expander("File Stats"):
					...
					"FileSize":text_file.size,"FileType":text_file.type}
					st.write(file_details)
		...
		# Extraction Process
			with dcol2:
				with st.beta_expander("Metadata"):
					...
					# st.write(pdf_info)
					# Convert to DataFrame
					df_file_details_with_pdf = pd.DataFrame(list(pdf_info.items()),
						columns=["Meta Tags","Value"])
